...
function getFacByStreamId($id) {
	$sql = "select * from faculty where streamId=:id";
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("id", $id);
...
function addResponse() {
	...
	$sql = "INSERT INTO results (accountId, quizId, answerStatus, timeTaken, timestamp) VALUES (:accountId, :quizId, :answerStatus, :timeTaken, :timeStamp)";
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		...
		$stmt->bindParam("answerStatus", $response->optionsSelected);
		$stmt->bindParam("timeTaken", $response->timeTaken);
		$stmt->bindParam("timeStamp", $response->timestamp);
		$stmt->execute();
